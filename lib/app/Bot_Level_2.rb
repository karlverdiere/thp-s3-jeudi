...
#Appel du fichier .env grâce à la gems Dotenv
Dotenv.load
...
# Follow (le journaliste) puis like les 3 derniers tweets des journalistes inscrit dans le tableau ci dessous

journalist = ["@Pierr_Person", "@EugenieBastie", "@Xavier75"]

for i in (0..journalist.length-1)
	my_user = journalist[i][1..100] #cette commande sert à enlever le @


	client.follow(my_user) #Voici la commonde pour follow chacun des journalistes du tableau

	client.user_timeline(my_user).take(3).collect do |tweet| #commande qui affiche les tweets sur la console et les like
				if tweet.is_a?(Twitter::Tweet)
				  puts tweet.text
				  client.fav tweet # Met en favoris
				 end
		end
